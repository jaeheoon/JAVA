...
public class AMS2 {
	...
	private static AccountRepository mirepository = new AccountRepository();
	private static Account accounts;
	private static MinusAccount miAccount = new MinusAccount();
	...
	private static void createMiAccount() {
		...
		Account miAccount = new MinusAccount(owner, passwd, inputMoney, borrowInputMoney);
	...
	private static void addRestMoney() {
		...
		if(accounts!=null) {
			...
			long addRestMoney = accounts.deposit(addMoney);
	...
	private static void addMiRestMoney() {
		System.out.print("계좌번호를 입력하세요: ");
		String accountNum = scanner.nextLine();
		accounts = mirepository.searchAccount(accountNum);
		if(accounts!=null) {
			System.out.print("입금할 금액을 입력하세요: ");
			long addMoney = Long.parseLong(scanner.nextLine());
			long addRestMoney = miAccount.deposit(addMoney);
			System.out.println("입금 후 잔액은: " + addRestMoney);
		}
		else {
			System.out.println("계좌번호에 해당하는 계좌가 존재하지 않습니다..");
		...
		if(accounts != null) {
			...
			int checkNumPwd = accounts.getPasswd() ;
				if(checkNumPwd==accountNumPwd) {
					...
					long disRestMoney = accounts.withdraw(disMoney);
	...
	private static void widthMiRestMoney() {
		...
		if(accounts != null) {
			...
			if(checkNumPwd==accountNumPwd) {
				System.out.print("출금할 금액을 입력하세요: ");
				long disMoney = Long.parseLong(scanner.nextLine());
				long disRestMoney = miAccount.withdraw(disMoney);
				System.out.println("출금 후 잔액은: " + disRestMoney);
			}
			else {
				System.out.println("비밀번호가 다릅니다..");
	...
	private static void accountType() {
		System.out.println("계좌 종류(1: 입출금 계좌, 2: 마이너스 계좌)");
